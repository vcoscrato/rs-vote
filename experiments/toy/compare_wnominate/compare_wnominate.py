...
from rsvote.models import AlternatingWNominate, WNominate
...
def generate_synthetic_data(n_leg=200, n_votes=100, missing_rate=0.1, seed=42):
    ...
    true_spreads = np.random.uniform(2.0, 5.0, n_votes) * np.random.choice(
        [-1, 1], n_votes
    )
...
def plot_comparison(true_points, r_points, py_points, py_alt_points):
    ...
    fig, axes = plt.subplots(1, 4, figsize=(24, 15))
    ...
    axes[1].set_title(
        f"PyTorch Alternating vs R W-NOMINATE\nCorrelation: {corr_alt:.4f}"
    )
    ...
    axes[2].set_title(
        f"PyTorch vs True Underlying Model\nCorrelation: {corr_py_true:.4f}"
    )
    ...
    sns.kdeplot(
        r_norm, ax=axes[3], label="R wnominate", fill=True, alpha=0.3, color="red"
    )
    sns.kdeplot(
        py_norm,
        ax=axes[3],
        label="PyTorch W-NOMINATE",
        fill=True,
        alpha=0.3,
        color="blue",
    )
...
def main():
    ...
    log(f"Dataset Details: {votes_matrix.shape[0]} Legislators, {votes_matrix.shape[1]} Roll Calls.")

    # 2. Run models
    log("Running models (R and PyTorch equivalents)...")
    ...
    valid_idx = ~np.isnan(r_ideal_points) & ~np.isnan(py_ideal_points) & ~np.isnan(py_alt_ideal_points)
    r_val = r_ideal_points[valid_idx]
    py_val = py_ideal_points[valid_idx]
    py_alt_val = py_alt_ideal_points[valid_idx]
    true_val = true_ideal_points[valid_idx]
    
    def normalize(x):
        x = x - np.mean(x)
        return x / np.max(np.abs(x))
    
    r_norm = normalize(r_val)
    py_norm = normalize(py_val)
    py_alt_norm = normalize(py_alt_val)
    
    if np.corrcoef(r_norm, py_norm)[0, 1] < 0: py_norm = -py_norm
    if np.corrcoef(r_norm, py_alt_norm)[0, 1] < 0: py_alt_norm = -py_alt_norm
    if np.corrcoef(true_val, r_norm)[0, 1] < 0: true_val = -true_val
        
    corr = np.corrcoef(r_norm, py_norm)[0, 1]
    corr_alt = np.corrcoef(r_norm, py_alt_norm)[0, 1]
    corr_true = np.corrcoef(true_val, py_norm)[0, 1]
    
    log("\n--- Pearson Correlations ---")
    log(f"R wnominate vs PyTorch W-NOMINATE: {corr:.4f}")
    log(f"R wnominate vs PyTorch Alternating: {corr_alt:.4f}")
    log(f"True Underlying Model vs PyTorch W-NOMINATE: {corr_true:.4f}")
