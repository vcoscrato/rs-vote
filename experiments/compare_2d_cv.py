...
def generate_synthetic_2d_data(n_leg=200, n_votes=150, missing_rate=0.1, beta=15.0, seed=42):
    """
    ...
    true_spreads = (true_spreads / true_spreads_norms) * np.random.uniform(1.0, 5.0, (n_votes, 1))
...
def plot_2d_comparisons(true_pts, r_pts, py_no_cv, py_cv):
    ...
    def plot_scatter(ax, source_pts, title, color):
        ax.scatter(true_norm[:, 0], true_norm[:, 1], c='gray', alpha=0.3, label='Ground Truth')
        ax.scatter(source_pts[:, 0], source_pts[:, 1], c=color, alpha=0.8, marker='x', label='Recovered')
        
        # Draw lines to show displacement
        for i in range(len(true_norm)):
            ax.plot([true_norm[i, 0], source_pts[i, 0]], 
        ...
        ax.text(0.05, 0.95, f"MSE: {dist:.4f}\nDim 1 Corr: {corr1:.3f}\nDim 2 Corr: {corr2:.3f}", 
                transform=ax.transAxes, verticalalignment='top', 
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

    plot_scatter(axes[0, 0], r_aligned, "R: wnominate (No CV)", color="red")
    plot_scatter(axes[0, 1], py_no_cv_aligned, "Python: PyTorch W-NOMINATE (No CV)", color="blue")
    plot_scatter(axes[1, 0], py_cv_aligned, "Python: PyTorch W-NOMINATE (With 10% CV Holdout)", color="purple")
    ...
    plt.tight_layout()
    output_path = os.path.join(os.path.dirname(__file__), "wnominate_2d_cv_comparison.png")
    plt.savefig(output_path, dpi=300)
    print(f"Plot saved to: {output_path}")

def main():
    print("--- 2D W-NOMINATE Parity & CV Experiment ---")
    
    # 1. Generate 2D data
    # (Using 150 legislators and 100 votes for a well-conditioned matrix)
    true_ideal_points, votes_matrix = generate_synthetic_2d_data(n_leg=150, n_votes=100, seed=42)
    ...
    plot_2d_comparisons(true_ideal_points, r_pts, py_no_cv_pts, py_cv_pts)
