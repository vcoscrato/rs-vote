...
class AlternatingWNominate(WNominate):
    ...
    def fit(self, X: RollCallMatrix, X_val: RollCallMatrix = None, epochs: int = 1500, lr: float = 0.05) -> 'AlternatingWNominate':
        ...
        for epoch in pbar:
            ...
            if X_val is not None and (epoch % 10 == 0 or epoch == epochs - 1):
                ...
                with torch.no_grad():
                    ...
                    pbar.set_postfix({'loss': f"{loss.item():.4f}", 'val_loss': f"{val_loss.item():.4f}", 'val_acc': f"{val_acc.item():.4f}", 'block': 'Legs' if optimize_legislators else 'Votes'})
            elif epoch % 10 == 0 or epoch == epochs - 1:
                pbar.set_postfix({'loss': f"{loss.item():.4f}", 'block': 'Legs' if optimize_legislators else 'Votes'})
