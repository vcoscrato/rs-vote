...
class RollCallMatrix:
    ...
    def __init__(
        self,
        votes: Union[np.ndarray, pd.DataFrame],
        legislators: Optional[pd.DataFrame] = None,
        rollcalls: Optional[pd.DataFrame] = None
    ):
        """
        ...
        if self.legislators is not None and len(self.legislators) != self.n_legislators:
            raise ValueError("Legislators metadata must match the number of rows in votes.")
            
        if self.rollcalls is not None and len(self.rollcalls) != self.n_votes:
            raise ValueError("Rollcalls metadata must match the number of columns in votes.")
    ...
    def train_test_split(self, test_size: float = 0.1, random_state: Optional[int] = None):
        """
